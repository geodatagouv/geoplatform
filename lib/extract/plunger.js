...
class SuperPlunger extends Plunger {
    ...
    return this.createTempDirectory()
      .then(path => {
        return new Promise((resolve, reject) => {
          ...
          this
            .pipeWithResponse(through.obj((chunk, enc, cb) => {
  ...
  decompressArchive() {
    if (this.decompressedDirectoryPath) {
      return Promise.resolve(this.decompressedDirectoryPath)
    }
    if (!this.archivePath) {
      return Promise.reject(new Error('`archivePath` is not defined'))
    }
    ...
    if (this.archive === 'rar') {
      decompressProcess = execAsync('unrar x archive.rar decompressed/', {cwd: this.tempDirectoryPath})
    }
  ...
  listFiles() {
    if (!this.decompressedDirectoryPath) {
      return Promise.reject(new Error('No iterable path found'))
    }
    ...
    return new Promise((resolve, reject) => {
      findit(this.decompressedDirectoryPath)
        .on('file', file => {
          ...
          if (shortFileName.match(/\.(shp|tab|mif)$/i)) {
            datasets.push(shortFileName)
          }
        })
