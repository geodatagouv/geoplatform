...
function isPublicURL(location) {
  ...
  if (!urlObj.is('url') || urlObj.is('relative')) {
    return false
  }
  ...
  if (!urlObj.is('ip') && !urlObj.is('name')) {
    return false
  }
  ...
  if (urlObj.is('ip')) {
    ...
    if (range && rangesToExclude.includes(range)) {
      return false
    }
  }

  if (urlObj.is('name')) {
    if (!tld.tldExists(urlObj.hostname())) {
      return false
    }

    if (urlObj.hostname().endsWith('ader.gouv.fr')) {
      return false
    }
  }
