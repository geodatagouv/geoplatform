...
const {mapValues, isNumber, isArray, clone, forEach, take} = require('lodash')
...
module.exports = function (searchQuery, catalogName) {
  ...
  const resolvers = {
    results: () => {
      return ConsolidatedRecord.collection.find(query, {
        projection: {
          recordId: 1,
          'metadata.title': 1,
    ...
    query: () => {
      return {facets: facetsFromQuery, limit, offset}
    }
