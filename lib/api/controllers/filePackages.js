...
const Plunger = require('../../extract/plunger')
const {strLeftBack} = require('underscore.string')
const {get, find, includes} = require('lodash')
...
exports.loadLayer = function (req, res, next) {
  ...
  const datasets = get(req.linkCheck, 'result.archive.datasets', [])

  if (req.params.layerName) {
    const foundLayer = find(datasets, dataset => dataset.includes(req.params.layerName))
    if (foundLayer) {
...
exports.prepateFilePackageDownload = function (req, res, next) {
  ...
  if (!req.linkCheck.result.available) {
    return notFound()
  }

  req.plunger = new Plunger(req.linkCheck.linkLocation, {abort: 'never'})
